...
//From
public final class PerlinNoise<I> implements FloatFunc<I> {
    private static final Interval OUTPUT_INTERVAL = new Interval(-.68f, .68f);

    public static final PercentileTable PERCENTILE_TABLE = PercentileTable.deserialize(
        Helper.getResource("/perlinPercentileTable")
    );
    ...
    public double getOutput(I input) {
        ...
        double value = rawNoise.noise(
            posInfo.getX(input)*inverseScale,
            posInfo.getZ(input)*inverseScale
        );

        //very rarely itll output extreme values that needs to be clamped
    ...
    //From https://mrl.cs.nyu.edu/~perlin/experiments/packing/render/Noise.java
    ...
    private static final class RawNoise {
        ...
        public RawNoise(int seed) {
            ...
            for (i = 0; i < B; i++) {
                p[i] = i;
                g1[i] = 2 * r.nextFloat() - 1;

                do {
                    u = 2 * r.nextFloat() - 1;
                    v = 2 * r.nextFloat() - 1;
                } while (u * u + v * v > 1 || Math.abs(u) > 2.5 * Math.abs(v) || Math.abs(v) > 2.5 * Math.abs(u) || Math.abs(Math.abs(
                    u) - Math.abs(v)) < .4);
                g2[i][0] = u;
                g2[i][1] = v;
                normalize2(g2[i]);
                ...
                } while (u * u + v * v + w * w > 1 || Hi > 4 * Lo || Math.min(
                    Math.abs(U - V),
            ...
            points[3][0] = points[3][1] = points[3][2] = (double)Math.sqrt(1. / 3);
            double r2 = (double)Math.sqrt(1. / 2);
            double s = (double)Math.sqrt(2 + r2 + r2);
        ...
        public double noise(double x, double y, double z) {
            ...
            bx = (int) Math.IEEEremainder((double)Math.floor(x), B);
            if (bx < 0)
                bx += B;
            rx0 = x - (double)Math.floor(x);
            rx1 = rx0 - 1;

            by = (int) Math.IEEEremainder((double)Math.floor(y), B);
            if (by < 0)
                by += B;
            ry0 = y - (double)Math.floor(y);
            ry1 = ry0 - 1;

            bz = (int) Math.IEEEremainder((double)Math.floor(z), B);
            if (bz < 0)
                bz += B;
            rz = z - (double)Math.floor(z);
