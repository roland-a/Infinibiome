...
public final class DarkOakGen extends TreeGenBase {
    ...
    private static final IPlantable SAPLING = (IPlantable) Blocks.DARK_OAK_SAPLING;

    public DarkOakGen(DependencyInjector di) {
        super(di, Infinibiome.MOD_ID + ":darkOak");

        config = this.<HugeTreeFeatureConfig>initConfig()
            .setFeature(
                new FixedDarkOakTreeFeature()
            )
            .setConfigFunc(
                (pos, height, random) ->
            ...
            .onlyGrowIn2x2Config()
            .setDensity(
                FloatFunc.constFunc(1)
            )
            .setRegionRate(
                TreeHelper.COMMON_REGION_RATE / 4
            )
            .noExtraConditions();
    }
                ...
                if (!isSoil(generationReader, blockpos, configIn.getSapling())) {
                    ...
                    for(int j2 = 0; j2 < i; ++j2) {
                        ...
                        if (isAirOrLeaves(generationReader, blockpos1)) {
                            this.setLog(generationReader, rand, blockpos1, p_225557_4_, boundingBoxIn, configIn);
                            this.setLog(generationReader, rand, blockpos1.east(), p_225557_4_, boundingBoxIn, configIn);
                            this.setLog(generationReader, rand, blockpos1.south(), p_225557_4_, boundingBoxIn, configIn);
                            this.setLog(generationReader, rand, blockpos1.east().south(), p_225557_4_, boundingBoxIn, configIn);
                        }
                    }
                    ...
                    for(int j3 = -2; j3 <= 0; ++j3) {
                        for(int i4 = -2; i4 <= 0; ++i4) {
                            int l4 = -1;
                            this.setLeaf(generationReader, rand, new BlockPos(k1 + j3, i2 + l4, l1 + i4), p_225557_5_, boundingBoxIn, configIn);
                            this.setLeaf(generationReader, rand, new BlockPos(1 + k1 - j3, i2 + l4, l1 + i4), p_225557_5_, boundingBoxIn, configIn);
                            this.setLeaf(generationReader, rand, new BlockPos(k1 + j3, i2 + l4, 1 + l1 - i4), p_225557_5_, boundingBoxIn, configIn);
                            this.setLeaf(generationReader, rand, new BlockPos(1 + k1 - j3, i2 + l4, 1 + l1 - i4), p_225557_5_, boundingBoxIn, configIn);
                    ...
                    if (rand.nextBoolean()) {
                        this.setLeaf(generationReader, rand, new BlockPos(k1, i2 + 2, l1), p_225557_5_, boundingBoxIn, configIn);
                        this.setLeaf(generationReader, rand, new BlockPos(k1 + 1, i2 + 2, l1), p_225557_5_, boundingBoxIn, configIn);
                        this.setLeaf(generationReader, rand, new BlockPos(k1 + 1, i2 + 2, l1 + 1), p_225557_5_, boundingBoxIn, configIn);
                        this.setLeaf(generationReader, rand, new BlockPos(k1, i2 + 2, l1 + 1), p_225557_5_, boundingBoxIn, configIn);
                    }

                    for(int k3 = -3; k3 <= 4; ++k3) {
                        for(int j4 = -3; j4 <= 4; ++j4) {
                            if ((k3 != -3 || j4 != -3) && (k3 != -3 || j4 != 4) && (k3 != 4 || j4 != -3) && (k3 != 4 || j4 != 4) && (Math.abs(k3) < 3 || Math.abs(j4) < 3)) {
                                this.setLeaf(generationReader, rand, new BlockPos(k1 + k3, i2, l1 + j4), p_225557_5_, boundingBoxIn, configIn);
                            }
                        }
                    }
                        ...
                        for(int k4 = -1; k4 <= 2; ++k4) {
                            if ((l3 < 0 || l3 > 1 || k4 < 0 || k4 > 1) && rand.nextInt(3) <= 0) {
                                ...
                                for(int l2 = 0; l2 < i5; ++l2) {
                                    this.setLog(generationReader, rand, new BlockPos(j + l3, i2 - l2 - 1, l + k4), p_225557_4_, boundingBoxIn, configIn);
                                }

                                for(int j5 = -1; j5 <= 1; ++j5) {
                                    for(int i3 = -1; i3 <= 1; ++i3) {
                                        this.setLeaf(generationReader, rand, new BlockPos(k1 + l3 + j5, i2, l1 + k4 + i3), p_225557_5_, boundingBoxIn, configIn);
                                    }
                                }
                                ...
                                for(int k5 = -2; k5 <= 2; ++k5) {
                                    for(int l5 = -2; l5 <= 2; ++l5) {
                                        if (Math.abs(k5) != 2 || Math.abs(l5) != 2) {
                                            this.setLeaf(generationReader, rand, new BlockPos(k1 + l3 + k5, i2 - 1, l1 + k4 + l5), p_225557_5_, boundingBoxIn, configIn);
                                        }
                                    }
                                }
                            }
