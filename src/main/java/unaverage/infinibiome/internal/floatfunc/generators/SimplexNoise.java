...
public final class SimplexNoise<T> implements FloatFunc<T> {
    public static final PercentileTable PERCENTILE_TABLE = PercentileTable.deserialize(
        Helper.getResource("/simplexPercentileTable")
    );

    private static final Interval OUTPUT_INTERVAL = new Interval(-1, 1);
    private static final Interval RANDOM_OFFSET_INTERVAL = new Interval(-100_000, 100_000);

    private final RawSimplexNoise rawSimplexNoise;
    private final double inverseScale;
    private final PosInfo<T> posInfo;

    //helps get rid of strange effects when stacking multiple noises
    private final int xOffset;
    private final int zOffset;


    public SimplexNoise(Seed seed, double scale, PosInfo<T> posInfo){
        Validate.isTrue(scale >= 0);

        seed = seed.newSeed("simplexNoise");

        Random random = new Random(seed.getAsInt());

        this.rawSimplexNoise = new RawSimplexNoise(random.nextInt());
        this.inverseScale = 1/scale;
        this.posInfo = posInfo;

        this.xOffset = (int)Interval.PERCENT.mapInterval(random.nextDouble(), RANDOM_OFFSET_INTERVAL);
        this.zOffset = (int)Interval.PERCENT.mapInterval(random.nextDouble(), RANDOM_OFFSET_INTERVAL);
    }

    @Override
    public double getOutput(T input) {
        //faster than doing posInfo.getX(input)/scale
        return rawSimplexNoise.noise(
            (posInfo.getX(input) + xOffset) * inverseScale,
            (posInfo.getZ(input) + zOffset) * inverseScale
        );
    }

    public double getOutput(T input, double extraScale) {
        assert extraScale > 0;

        return rawSimplexNoise.noise(
            (posInfo.getX(input) + xOffset) * inverseScale * (1/extraScale),
            (posInfo.getZ(input) + zOffset) * inverseScale * (1/extraScale)
        );
    }
    ...
    //from https://git.finf.uni-hannover.de/Schoko/ASE/-/blob/master/SimplexNoise_octave.java
    static final class RawSimplexNoise {// Simplex noise in 2D, 3D and 4D
        ...
        private static short p_supply[] = {151,160,137,91,90,15, //this hasValue all the numbers between 0 and 255, these are put in a random order depending upon the seed
        ...
        public RawSimplexNoise(int seed) {
            p=p_supply.clone();
        ...
        private static final double F2 = (double)(0.5*(Math.sqrt(3.0)-1.0));
        private static final double G2 = (double)((3.0-Math.sqrt(3.0))/6.0);
        private static final double F3 = (double)(1.0/3.0);
        private static final double G3 = (double)(1.0/6.0);
        private static final double F4 = (double)((Math.sqrt(5.0)-1.0)/4.0);
        private static final double G4 = (double)((5.0-Math.sqrt(5.0))/20.0);
        ...
        public double noise(double xin, double yin, double zin) {
            ...
            // Skew the input space to determine which simplex cell we're in
            double s = (xin+yin+zin)*F3; // Very nice and weightedgpa.infinibiome.api.noise.operation skew factor for 3D
            int i = fastfloor(xin+s);
            int j = fastfloor(yin+s);
